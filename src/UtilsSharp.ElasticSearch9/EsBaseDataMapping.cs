...
namespace UtilsSharp.ElasticSearch9
{
    ...
    public abstract class EsBaseDataMapping
    {
        ...
        public abstract void Create(ElasticsearchClient client, string index);
        ...
        /// <param name="client">es客户端</param>
        /// <param name="index">索引名称</param>
        public abstract void EntityMapping(ElasticsearchClient client, string index);
        ...
        /// Es客户端
        /// </summary>
        public ElasticsearchClient EsClient => EsClientByIndex();
        ...
        /// <returns></returns>
        internal ElasticsearchClient GetClient(ElasticSearchSetting setting)
        {
            if (!EsClientProvider.ClientDictionary.ContainsKey(setting.EsHttpAddress))
            {
                var currentIndexClientDictionary = new ConcurrentDictionary<string, ElasticsearchClient>();
        ...
        /// </summary>
        /// <param name="index">索引名称</param>
        public ElasticsearchClient EsClientByIndex(string index = "")
        {
            if (CurSetting == null)
            {
                if (Setting == null)
        ...
        /// <param name="client">es客户端</param>
        /// <param name="index">索引名称</param>
        private void RunEntityMapping(ElasticsearchClient client, string index)
        {
        ...
        private void RunBindAliasIndex(ElasticsearchClient client, string index)
        {
            lock (EsBaseAliasIndexBindLock)
            {
                var allAliasIndexes = new List<string>();
                var isHave = false;
                if (EsClientProvider.AliasIndexBindDictionary.ContainsKey(index))
                {
                    EsClientProvider.AliasIndexBindDictionary.TryGetValue(index, out allAliasIndexes);
                    isHave = true;
                }
                allAliasIndexes = allAliasIndexes ?? new List<string>();
                var addAliasIndexes = new List<string>();

                //过滤未添加的别名
                if (AppendBindAliasIndexes != null && AppendBindAliasIndexes.Any(w => !allAliasIndexes.Contains(w)) || !isHave)
                {
                    //重新获取索引的全部别名
                    var indices = client.Indices.GetAlias(index, new Names())?.Aliases;
                    indices?.Keys.ToList().ForEach(key =>
                    {
                        indices[key].Aliases.Keys.ToList().ForEach(alias =>
                        {
                            if (!allAliasIndexes.Contains(alias))
                            {
                                allAliasIndexes.Add(alias);
                            }
                        });
                    });
                    if (AppendBindAliasIndexes != null && AppendBindAliasIndexes.Any(w => !allAliasIndexes.Contains(w)))
                    {
                        addAliasIndexes = AppendBindAliasIndexes.Where(w => !allAliasIndexes.Contains(w)).ToList();
                    }
                }
                //把默认别名附加进去
                if (!string.IsNullOrWhiteSpace(index) && !string.IsNullOrWhiteSpace(AliasIndex) && !addAliasIndexes.Contains(AliasIndex) && !allAliasIndexes.Contains(AliasIndex))
                {
                ...
                {
                    foreach (var addAliasIndex in addAliasIndexes)
                    {
                        client.Indices.PutAlias(index, addAliasIndex);
                    }
                    allAliasIndexes.AddRange(addAliasIndexes);
                }
                allAliasIndexes = allAliasIndexes.Distinct().ToList();
                if (isHave)
                {
                    EsClientProvider.AliasIndexBindDictionary[index] = allAliasIndexes;
                }
                else
                {
                    EsClientProvider.AliasIndexBindDictionary.TryAdd(index, allAliasIndexes);
                }
            }
        }
