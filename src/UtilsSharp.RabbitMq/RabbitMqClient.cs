...
namespace UtilsSharp.RabbitMq
{
    ...
    public partial class RabbitMqClient
    {
        ...
        public void Received(string queueName, Action<string> callback,Action<string> errorCallback = null)
        {
            Task.Run(() =>
            {
                using var channel = GetChannel();
        ...
        public void BatchReceived(string queueName, Action<List<string>> callback, int batchCount = 50, Action<string> errorCallback = null, int millisecondsDelay = 100)
        {
            Task.Run(() =>
            {
        ...
        private static void BatchReceivedHandle(Action<List<string>> callback, List<MessageAskModel> rabbitMessage)
        {
            if (rabbitMessage is not {Count: > 0}) return;
            try
            {
    ...
    public partial class RabbitMqClient
    {
        ...
        /// <param name="callback">回调方法</param>
        /// <param name="errorCallback">错误回调方法</param>
        public async Task ReceivedAsync(string queueName, Action<string> callback,Action<string> errorCallback = null)
        {
            await Task.Run(() =>
            {
                using var channel = GetChannel();
        ...
        /// <param name="errorCallback">错误回调方法</param>
        /// <param name="millisecondsDelay">如果队列无数据休眠时间：毫秒</param>
        public async Task BatchReceivedAsync(string queueName, Action<List<string>> callback, int batchCount = 50, Action<string> errorCallback = null, int millisecondsDelay=100)
        {
            var rabbitMessage = new List<MessageAskModel>();
            while (true)
            {
        ...
        /// <param name="callback">回调方法</param>
        /// <param name="rabbitMessage">批量消息</param>
        private static async Task BatchReceivedHandleAsync(Action<List<string>> callback, List<MessageAskModel> rabbitMessage)
        {
            await Task.Run(() =>
            {
                if (rabbitMessage is not {Count: > 0}) return;
