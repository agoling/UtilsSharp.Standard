...
namespace UtilsSharp.ElasticSearch.Extension
{
    ...
    public partial class EsBaseAppService : IEsBaseAppService
    {
        ...
        /// <returns></returns>
        public ValueTask<BaseResult<string>> Save<T1, T2>(T1 request, Func<T2,IIndexResponse> saveFunc) where T1 : EsBaseSaveRequest
        {
            ...
            {
                result.SetError(r.Result == Result.Created ? $"添加失败:{r.ServerError}" : $"修改失败:{r.ServerError}");
            }
            return new ValueTask<BaseResult<string>>(result);
        }
    ...
    public partial class EsBaseAppService 
    {
        ...
        /// <returns></returns>
        public async ValueTask<BaseResult<string>> SaveAsync<T1, T2>(T1 request, Func<T2,Task<IIndexResponse>> saveFuncAsync) where T1 : EsBaseSaveRequest
        {
            var result = new BaseResult<string>();
            var vr = ValidationHelper.IsValid(request);
            if (!vr.IsValid)
        ...
        /// <param name="saveFuncAsync">保存方法</param>
        /// <returns></returns>
        public async ValueTask<BaseResult<int>> BulkSaveAsync<T1, T2>(List<T1> request, Func<List<T2>, Task<IBulkResponse>> saveFuncAsync) where T1 : EsBaseSaveRequest
        {
            var result = new BaseResult<int>();
        ...
        /// <returns></returns>
        public async ValueTask<BaseResult<int>> IncrementModifyAsync<T>(EsBaseIncrementModifyRequest request, Func<string, Dictionary<string, object>, Task<IUpdateResponse<T>>> incrementModifyFuncAsync) where T : class
        {
            var result = new BaseResult<int>();
            var vr = ValidationHelper.IsValid(request);
            if (!vr.IsValid)
        ...
        /// <returns></returns>
        public async ValueTask<BaseResult<int>> DeleteAsync(EsBaseIdsRequest request, Func<string[], Task<IBulkResponse>> deleteFuncAsync)
        {
            var result = new BaseResult<int>();
            var vr = ValidationHelper.IsValid(request);
            if (!vr.IsValid)
