...
namespace UtilsSharp
{
    ...
    public class ChinesePinyinHelper
    {
        ...
        /// <returns></returns>
        public static string GetPinyinInitials(string chineseStr)
        {
            var i = 0;
            var strResult = Empty;
            ...
            var unicodeBytes = unicode.GetBytes(chineseStr);
        ...
        /// <param name="chineseStr">汉字字符串</param>
        /// <returns></returns>
        public string GetPinyin(string chineseStr)
        {
            var reg = new Regex("^[\u4e00-\u9fa5]$");//验证是否输入汉字
            var pystr = "";
            var mChar = chineseStr.ToCharArray();//获取汉字对应的字符数组
            foreach (var item in mChar)
            {
                //如果输入的是汉字
                if (reg.IsMatch(item.ToString()))
                {
                    var arr = Encoding.Default.GetBytes(item.ToString());
                    int m1 = arr[0];
                    int m2 = arr[1];
